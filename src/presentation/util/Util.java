...
public class Util {
	...
	public static LocalDate localDateForString(String date) { // pattern:
																// "MM/dd/yyyy"
		return LocalDate.parse(date, DateTimeFormatter.ofPattern(DATE_PATTERN));
	}

	public static String localDateAsString(LocalDate date) { // pattern:
																// "MM/dd/yyyy"
	...
	public static List<CartItemPres> cartItemsToCartItemPres(List<CartItem> list) {
		...
		return list.stream().map(c -> {
			...
			CartItemPres p = new CartItemPres();
			p.setCartItem(d);
			return p;
		}).collect(Collectors.toList());
	...
	public static double computeTotal(List<CartItem> list) {
		DoubleSummaryStatistics summary = list.stream().collect(
