...
class DbClassOrder implements DbClass {
	...
	private String orderItemsQuery = "SELECT * FROM accountsdb.OrderItem oi INNER JOIN productsdb.product p ON oi.productid = p.productid where oi.orderid = ?";
	...
	private OrderImpl orderData = new OrderImpl();
	...
	private void submitOrderItem(OrderItem item) throws DatabaseException {
		...
		// implement
		submitOrderItemParams = new Object[] { item.getOrderId(), item.getProductId(), item.getQuantity(),
				item.getTotalPrice(), 0.0, 0.0 };
		submitOrderItemTypes = new int[] { Types.SMALLINT, Types.SMALLINT, Types.INTEGER, Types.DOUBLE, Types.DOUBLE,
				Types.DOUBLE };
	...
	private void populateOrderItems(ResultSet rs) throws DatabaseException {
		...
		try {
			while (rs.next()) {
				OrderItemImpl orderItemData = new OrderItemImpl(rs.getString("productname").trim(), Integer.parseInt(rs
						.getString("quantity").trim()), Double.parseDouble(rs.getString("priceperunit").trim()));

				orderItemData.setOrderId(Integer.parseInt(rs.getString("orderid").trim()));
				orderItemData.setOrderItemId(Integer.parseInt(rs.getString("orderitemid").trim()));
				orderItemData.setProductId(Integer.parseInt(rs.getString("productid").trim()));
				lst.add(orderItemData);
			}
			orderItems = lst;
	...
	private void populateOrderData(ResultSet resultSet) throws DatabaseException {
		...
		try {
			while (resultSet.next()) {
				...
				if(resultSet.getString("totalpriceamount") != null){
					orderData.setTotalPrice(Double.parseDouble(resultSet.getString("totalpriceamount").trim()));
				}
			}
		} catch (SQLException e) {
			throw new DatabaseException(e);
