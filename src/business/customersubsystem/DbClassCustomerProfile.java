...
class DbClassCustomerProfile implements DbClass, DbClassCustomerProfileForTest{
	...
	private String readDefaultCreditQuery = "SELECT * FROM accountsdb.altpayment where custid = ?";
	...
	public void populatePaymentInfo(ResultSet resultSet) throws DatabaseException {
		try {
			...
			if (resultSet.next()) {
				defaultCreditCard = new CreditCardImpl("test", resultSet.getString("expdate"),
						resultSet.getString("cardnum"), resultSet.getString("cardtype"));
			}
